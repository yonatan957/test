...
function Mission1(unit){
    let chiefOfStaff = unit.commandStructure.chiefOfStaff
    return `rank: ${chiefOfStaff.rank}, name: ${chiefOfStaff.name}, phon: ${chiefOfStaff.contact.phone}`
} 
...
function Mission3(NewDeployment, unit){
    unit.history.push({eventDescription: unit.currentDeployment.mission, eventDate: unit.currentDeployment.startDate})
    unit.currentDeployment = NewDeployment
    return unit
}

function Mission4(Firearm, unit){
    let weapon = unit.equipment.firearms.find(f => f.type === Firearm.type)
    if(weapon){
        weapon.quantity += Firearm.quantity
    }
    else{
...
function Mission5(unit) {
    return unit.trainingPrograms.reduce((total, training) => {
